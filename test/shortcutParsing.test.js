...
test('sumForDay drops samples dated to a different day than the target', () => {
  const values = '100\n200\n300';
  ...
  assert.equal(sumForDay(values, dates, '2026-07-19', dayFn), 500);
});

test('sumForDay falls back to summing everything when dates is missing or mismatched in length', () => {
  const values = '100\n200\n300';
  assert.equal(sumForDay(values, '', '2026-07-19', () => '2026-07-19'), 600);
  assert.equal(sumForDay(values, '19 Jul 2026 at 08:00', '2026-07-19', () => '2026-07-19'), 600);
});

test('sumForDay returns null when no sample actually matches the target day', () => {
  const values = '100\n200';
