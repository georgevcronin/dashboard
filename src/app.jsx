...
function S3({ s, recommendation, runRecommendation, bikeRecommendation, swimRecommendation, performanceTrend, onStartWorkout, onImport, onHistory, refresh }) {
  ...
  return (
    <section id="s3">
      ...
      {/* #12: fitness/fatigue/form trend, same fetch-after-summary rule as
          runRecommendation above. A deload nudge only ever appears here —
          this proposes, it never touches weeklyPlan itself. */}
...
function Onboarding({ onComplete, onOpenImport }) {
  ...
  const [trackingCategories, setTrackingCategories] = useState({ sleep: true, nutrition: true });
  ...
  const advance = async () => {
    ...
    try {
      ...
      if (step === 5) await api('profile', { method: 'POST', body: JSON.stringify({ trackingCategories }), throwOnError: true });
