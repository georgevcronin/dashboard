...
152. **Swimming & Biking — Full Parity with Running** *(Built, 2026-08-06)* — Swim/bike training load, ACWR, HR-zone prescription, session recommendation, and fatigue interference, at the same depth as the running subsystem (#95–113), reusing that subsystem's genuinely sport-agnostic math directly rather than re-deriving it: TRIMP (`dailyLoadsFromRuns`, Banister 1991/Morton et al. 1990) and Karvonen HR zones (`karvonen5Zones`) take only duration/HR/resting-HR/max-HR — nothing running-specific — so `functions/cardio.js` calls them as-is on swim/bike data. `functions/cardioRecommendation.js` mirrors `buildRunningRecommendation`'s decision logic (readiness, session type, duration) via the same underlying functions, with fresh reasoning prose per activity (`buildCardioReasoningString`) rather than reusing running's prose, which hardcodes "run" throughout. New `GET /cardio/recommendation?type=bike|swim`; surfaced as "Today's Ride"/"Today's Swim" cards in S3 alongside "Today's Run." Fatigue interference (#103's Gabbett-principle unified ceiling) extended in `hybridFatigue.js` with a per-modality muscle split (`CARDIO_MUSCLE_SPLIT`) — bike quad-dominant like running, swim genuinely different (lats/shoulders/triceps/core) — same reasoned-estimate rigor as running's existing 60/20/20 split, not a new citation. **Deliberately not extended:** #102's single-session-distance-spike injury model (validated specifically for running, Garmin-RUNSAFE) and VDOT pace zones (a running-economy formula) — both would overclaim evidence that doesn't exist for swim/bike; ACWR (validated across endurance sports generally) is the honest choice for injury-risk framing instead. **Storage:** no new schema — every non-running Strava activity has landed in `db.sports` (full structured date/duration/distance/HR/elevation) since before this feature existed; nothing downstream had ever read it. 22 new tests (`test/cardio.test.js`, `test/cardioRecommendation.test.js`, plus additions to `test/hybridFatigue.test.js`).
153. **Bug fix: `GET /run/recommendation` uncaught for any account with real run data** *(Fixed, 2026-08-06)* — Found while building #152: `functions/index.js` imported `computeRunningACWR` from `./runningLoad`, but that function is defined and exported by `./fatigue` — the import silently resolved to `undefined`. Calling it as a function threw inside the route handler; Express 4 doesn't auto-catch async route errors, and the handler had no try/catch, so the request just hung/errored for any account with `db.runs.length > 0` (the empty-runs case returns early and never reached the bug, which is why it went unnoticed). This means #146's "Today's Run" card and #146's zone-two weighting — both built earlier this same session — had never actually been reachable for an account with logged runs. One-line fix (correct import source); `/cardio/recommendation` was written with an explicit try/catch from the start specifically to not repeat this failure mode.
154. **#14: Tracking-Category Degradation Audit — AI Briefings/Newscasts/Weekly Review** *(Built, 2026-08-06)* — Follow-up to #151's checkboxes: audited every AI-prompt-builder (`generateMorningBriefing`, `generateNewscast`, `generateWeeklyReview` in `functions/index.js`) for places that referenced sleep/nutrition data unconditionally regardless of whether the account had turned those categories off. Found and fixed: the morning briefing's "Fuel" persona always wrote a nutrition prescription and always flagged "NOT LOGGED" for yesterday's nutrition/sleep even when the user never opted into tracking either; the afternoon/evening newscast's `nutritionNote` did the same; the weekly review's sleep/nutrition lines and `nutritionNote` did too, and — a distinct, more concrete bug — `bucketWorkingAttention` (`functions/weeklyReview.js`) was fed `nutritionDaysLogged: 0` (a real number, not `null`) for a nutrition-off account, which its own `!= null` gate doesn't protect against the way it correctly does for sleep, so "Nutrition logged 0/7 days" landed in `needsAttention` every single week regardless of whether the account ever asked to track nutrition. All three generators now resolve `trackingCategoriesFor(profile)` and omit the relevant data line, persona, or nag entirely rather than reporting a permanent gap. `functions/trackingCategories.js` gained the shared `resolveTrackingCategories()` — the one implementation now used by both the backend generators and `src/app.jsx`'s own render gate (previously two independent copies of the same migration logic, itself exactly the kind of drift #14 was auditing for; `src/app.jsx` now imports the backend module directly, the same cross-import pattern already established for `muscleTaxonomy.js`/`micronutrients.js`). **Not done in this pass:** onboarding step 5 (tracking-category selection) comes three steps after step 2 already asks for a sleep target and water target regardless of what gets selected later — a real ordering inconsistency, flagged here rather than reordering onboarding's step sequence blind, which risks touching more than this fix's scope.
155. **Goal Editing In-Place** *(Built, 2026-08-06)* — Before this, `profile.goals` could only be written by onboarding step 2 (`saveGoals` in `src/app.jsx`) — the Goals panel (`S8`, `src/sections/Goals.jsx`) only ever rendered goals read-only, with no add/edit/delete affordance anywhere post-onboarding; the only way to change an existing goal's priority, target, target date, or metric was to redo onboarding entirely (`onRestartSetup`), which re-asks every onboarding question, not just goals. Each goal row in `S8` now has an "Edit" button opening an inline form (priority, concrete-target toggle, metric, exercise/benchmark name, target, and target date — the same fields and controls onboarding's goal step uses, `prof-btn`/`ob-label`/`prof-input`, no new visual language) that saves via the existing `POST /profile` `{ goals: [...] }` endpoint, sending the full updated array (a full replace, same as `saveGoals` already does — `functions/index.js` merges `body.goals` in wholesale, not per-entry) so unedited goals travel through unchanged. Client-side validation mirrors `validateGoals()` (`functions/goalsAndActivities.js`, imported directly rather than duplicated) so Save is disabled until the draft would actually pass server validation. **Scope decision:** a goal's `type` is not editable — it determines which metrics are valid (`GOAL_METRICS` is keyed by type) and is effectively the goal's identity, so changing it is really deleting-and-recreating, not editing; adding a new goal or deleting an existing one from the panel itself was not part of this ask and remains unbuilt (still onboarding-only), flagged here rather than silently expanded into scope.
