...
62. **Scientific Evidence Library** *(Built, 2026-08-06)* — A new "Evidence" tab in the Wiki (alongside Concepts and Exercises), `EVIDENCE_LIBRARY` in `src/app.jsx`: 13 entries linking an algorithm to the citation(s) that actually justify it. Every citation quotes a source that already existed somewhere in the codebase — a code comment or `RUNNING_SCIENCE.md`'s own References section (which pre-dated this feature entirely and had never been surfaced to a user) — verbatim or near-verbatim, nothing reconstructed from partial memory of a paper. An algorithm with no real citation in its source simply isn't listed. The one deliberate exception: RIR effectiveness is included but labelled "named practitioner heuristic — not a peer-reviewed citation," because that's honestly what its source comment says it is, not dressed up as a study.
...
145. **Interactive First-Encounter Walkthrough** *(Built, 2026-08-05; extended to every panel + Settings, 2026-08-07)* — Spotlight/tooltip overlay that highlights a live control or region with a short caption and steps through a section's sequence, auto-triggering the first time (and only the first time, ever) a given user actually encounters that section — not just once at signup. Distinct mechanism from #76 ("Progressive Feature Discovery… contextual tips as history grows"): #76 is history-driven and ongoing, this is a one-shot per-section reveal keyed off first exposure, with a per-section `profile.walkthroughsSeen` flag and a Settings → Account "Replay Walkthrough" control to reset and re-trigger on demand. A confirmed, scoped exception to `PRODUCT.md` Design Principle #5 — see that file's note under Design Principle #5. Content now covers every dock section (Dispatch, Sleep, Training, Nutrition, Recovery, Body, Records, Goals, Social, Cycle, Running) plus Settings itself — Settings auto-fires the first time it's opened, the same first-encounter mechanism as the panels, which is a further explicit extension of the Design Principle #5 exception beyond the panels-only scope this feature originally shipped with (`PRODUCT.md`'s note has been updated to match). (Renumbered from #143 to #145 during the merge of PR #45 alongside PR #42, which had already claimed #143/#144 for the same next-available slot — nothing else renumbered.)
...
149. **Independent Tracking-Category Checkboxes** *(Built, 2026-08-08)* — Replaces `ECHELONS`' 3-preset radio (Training / Training+Recovery / Full System, stored as `profile.trackingLevel`) with two independent checkboxes, Recovery and Nutrition (`TRACKING_CATEGORIES`, `profile.trackingCategories`) — Training itself always on. Expresses combinations the old 3 presets couldn't (e.g. Training + Nutrition, no Recovery). Migrated at read time only, matching `withNewDefaultsAppended`'s pattern for panels — `trackingCategoriesFor()` prefers a saved `trackingCategories` object, falling back to mapping an existing `trackingLevel` string, never rewriting stored data in place. `functions/trackingCategories.js`'s `validateTrackingCategories` guards `POST /profile`'s write side, unit-tested (`test/trackingCategories.test.js`, which also locks the frontend migration map's expected behavior since `src/app.jsx` has no test runner reaching it). **Not included:** #14, the actual "does Training-only mode degrade every panel gracefully" audit the checkboxes make more reachable (any combination, not just the old three) — every panel reading sleep/nutrition data beyond the two already-gated ones (S2/S4) needs checking, per `.design/feature-brainstorm/IDEA_LIST_TRIAGE.md`'s own framing: "the real work, not the toggle UI." Flagged, not silently folded into this entry.
150. **#14: Tracking-Category Degradation Audit — AI Briefings/Newscasts/Weekly Review** *(Built, 2026-08-08)* — Follow-up to #149's checkboxes: audited every AI-prompt-builder (`generateMorningBriefing`, `generateNewscast`, `generateWeeklyReview` in `functions/index.js`) for places that referenced sleep/nutrition data unconditionally regardless of whether the account had turned those categories off. Found and fixed: the morning briefing's "Fuel" persona always wrote a nutrition prescription and always flagged "NOT LOGGED" for yesterday's nutrition/sleep even when the user never opted into tracking either; the afternoon/evening newscast's `nutritionNote` did the same; the weekly review's sleep/nutrition lines and `nutritionNote` did too, and — a distinct, more concrete bug — `bucketWorkingAttention` (`functions/weeklyReview.js`) was fed `nutritionDaysLogged: 0` (a real number, not `null`) for a nutrition-off account, which its own `!= null` gate doesn't protect against the way it correctly does for sleep, so "Nutrition logged 0/7 days" landed in `needsAttention` every single week regardless of whether the account ever asked to track nutrition. All three generators now resolve `trackingCategoriesFor(profile)` and omit the relevant data line, persona, or nag entirely rather than reporting a permanent gap. `functions/trackingCategories.js` gained the shared `resolveTrackingCategories()` — the one implementation now used by both the backend generators and `src/app.jsx`'s own render gate (previously two independent copies of the same migration logic, itself exactly the kind of drift #14 was auditing for; `src/app.jsx` now imports the backend module directly, the same cross-import pattern already established for `muscleTaxonomy.js`/`micronutrients.js`). **Not done in this pass:** onboarding step 5 (tracking-category selection) comes three steps after step 2 already asks for a sleep target and water target regardless of what gets selected later — a real ordering inconsistency, flagged here rather than reordering onboarding's step sequence blind, which risks touching more than this fix's scope.
151. **Goal-Aware Zone 2 Cardio Weighting** *(Built, 2026-08-08)* — Extends #97 (Physiological Target-Based Running Prescriptions): when `profile.goals` includes an active `fatLoss` goal, `sessionTypeByReadiness` (`functions/runningRecommendation.js`) biases session-type selection at readiness ≥70 toward steady-state Zone 2 work (`runningPrescription.js`'s `z2`, 60–70% HR reserve — "aerobic threshold, most long runs, general base aerobic," the intensity most associated with fat oxidation as fuel) instead of the higher-intensity intervals/tempo it would otherwise mix in, with the reasoning stated plainly rather than asserted. No change below readiness 70 — recovery/easy there are already Zone 1/2-equivalent. (The original commit also added a minimal "Today's Run" frontend card — skipped here, since S11's full running panel, built later, already surfaces `GET /run/recommendation` in much more depth.)
152. **Bug fix: `GET /run/recommendation` uncaught for any account with real run data** *(Fixed, 2026-08-08)* — `functions/index.js` imported `computeRunningACWR` from `./runningLoad`, but that function is defined and exported by `./fatigue` — the import silently resolved to `undefined`. Calling it as a function threw inside the route handler; Express 4 doesn't auto-catch async route errors, and the handler had no try/catch, so the request just hung/errored for any account with `db.runs.length > 0` (the empty-runs case returns early and never reached the bug, which is why it went unnoticed). Found while auditing this same unported branch, which had independently hit and documented the identical bug. One-line fix (correct import source).
153. **Swimming & Biking — Full Parity with Running** *(Built, 2026-08-08)* — Swim/bike training load, ACWR, HR-zone prescription, session recommendation, and fatigue interference, at the same depth as the running subsystem (#95–113), reusing that subsystem's genuinely sport-agnostic math directly rather than re-deriving it: TRIMP (`dailyLoadsFromRuns`, Banister 1991/Morton et al. 1990) and Karvonen HR zones (`karvonen5Zones`) take only duration/HR/resting-HR/max-HR — nothing running-specific — so `functions/cardio.js` calls them as-is on swim/bike data. `functions/cardioRecommendation.js` mirrors `buildRunningRecommendation`'s decision logic (readiness, session type, duration) via the same underlying functions, with fresh reasoning prose per activity (`buildCardioReasoningString`) rather than reusing running's prose, which hardcodes "run" throughout. New `GET /cardio/recommendation?type=bike|swim`. Fatigue interference (#103's Gabbett-principle unified ceiling) extended in `hybridFatigue.js` with a per-modality muscle split (`CARDIO_MUSCLE_SPLIT`) — bike quad-dominant like running, swim genuinely different (lats/shoulders/triceps/core) — same reasoned-estimate rigor as running's existing 60/20/20 split, not a new citation. **Deliberately not extended:** #102's single-session-distance-spike injury model (validated specifically for running, Garmin-RUNSAFE) and VDOT pace zones (a running-economy formula) — both would overclaim evidence that doesn't exist for swim/bike; ACWR (validated across endurance sports generally) is the honest choice for injury-risk framing instead. **Storage:** no new schema — every non-running Strava activity has landed in `db.sports` (full structured date/duration/distance/HR/elevation) since before this feature existed; nothing downstream had ever read it. **Frontend surface skipped:** the original commit's "Today's Ride"/"Today's Swim" S3 cards — S12/S13, full dedicated panels built independently after this branch diverged, already surface `GET /cycling/recommendation` and `GET /swim/recommendation` in far more depth than a minimal S3 card would. 22 new tests (`test/cardio.test.js`, `test/cardioRecommendation.test.js`, plus additions to `test/hybridFatigue.test.js`).
154. **Micronutrient Tracking** *(Built, 2026-08-08)* — Vitamins/minerals/fiber/sodium etc. alongside the existing macro tracking, on every logging path. `functions/micronutrients.js` extracts whichever fields a barcode-scanned product actually reports from OpenFoodFacts' `nutriments` object (`/food/barcode` — real measured data, discarded before this) — no fabricated zeros for fields a product doesn't report. Per direct instruction, also extended to the AI-estimate paths (`/nutrition/analyze`'s meal-photo, label-photo, and text-description modes) with an explicit, unmissable "estimated, may not be accurate" label everywhere it's shown, distinct from the barcode path's real-data framing — the two are never presented at the same confidence level. Raw values + EU-NRV-relative percent (`pctRDA`), never a score/badge, per `PRODUCT.md`'s anti-gamification stance. Shown on the current (not-yet-logged) entry in S4's log form and as a daily rollup under S4's macro bars, both absent entirely on a day/entry with nothing micronutrient-bearing rather than a block of zeros.
155. **Standalone Exercise Rating** *(Built, 2026-08-08)* — Extends #142's pairwise comparison prompt beyond the finish-workout screen: a "Rate Exercises" option in All-Time Bests (S7) pairs exercises from the athlete's whole logged history instead of just the session that was just finished, via the same `detectComparisonCandidates` (`functions/exercisePreferenceRanking.js`, one candidate per shared primary muscle) called with the full history on both sides. Posts to the same `POST /preferences/compare` endpoint — no backend changes needed, since both were already generic to where the comparison came from. Collapsed behind a toggle, one pair at a time, skippable, same interaction as the original prompt.
