...
141. **New-Account Safety** — Guarantee every new account starts genuinely empty: no other account's history is ever cloned into or visible from a fresh signup (closes the class of bug behind the account-data-mixing incident, commit `6b1ce27`). **Still open** — `loadForUser`'s top-level fallback was fixed by that commit, but the full audit (every path that seeds a doc for a uid with no existing doc, across the now-much-larger username/follow/compare/group-session surface) hasn't been done.
