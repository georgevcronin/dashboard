...
- **Curl / Extension — Depth/ROM ruled N for the EMG-crediting mechanism specifically, not for lack of evidence.** Real, meta-analyzed literature on full-vs-lengthened-partial ROM training exists for both patterns (see `.design/feature-brainstorm/CURL_EXTENSION_ROM_LITERATURE_REVIEW.md` — Kassiano et al. 2023, a 2025 meta-analysis pooling biceps brachii/brachialis, Havers et al. 2025, and Wolf et al. 2025 covering both elbow flexors and extensors). But the finding is hypertrophy *equivalence* between full and lengthened-partial ROM (never worse, sometimes a small edge to partials in less-trained lifters, shrinking to null in trained ones), with at most a *regional* effect within one muscle (proximal vs. distal biceps) — not the muscle-to-muscle redistribution (more credit to X, less to Y) that Sagittal Angle/Rotation/Width all represent and that this column's Y/N has meant everywhere else in this table. There's no honest EMG percentage table to build from that finding. Recommended treatment (if built at all): a coaching-note-tier acknowledgment or a non-EMG-bearing Full/Lengthened-Partial logging toggle — a `§3`-style informational feature, not a `§17`-style stored parameter. Not yet built either way.
...
**`PressRowBuilder`/unified Press scale reconciliation — DONE.** The gap flagged above (Press-mode angle scale mismatch between the custom-exercise builder and the canonical Bench Press picker) was fixed directly: `PressRowBuilder` now uses the same -90..90 scale (via `benchPressEmgProfileForAngle`'s anchor-snapping) for Press, so a custom Press exercise gets the same muscle credit a canonical Bench Press entry at that angle would. Row's builder mode needed no change — its scale already matched. Also added: a live resolved-name preview (`matchExerciseName`) while picking an angle, baked into the generated exercise's own name, so custom Press/Row exercises read as e.g. "Barbell Incline Shoulder Press — 45°" in history instead of an indistinguishable "Barbell Press — 45°".

## 22. Row-family fold-in, completing what §15 deferred — DONE

§15 explicitly scoped itself to "only the pull-up/pulldown cluster, not the whole ~15-entry Row family," naming Bent-Over Row, Seated Cable Row, T-Bar Row, Meadows Row, Face Pull, Upright Row, and Kelso Shrug as left untouched. This pass picks up that deferred scope for every named entry that's actually an angle point on Row's existing 0-180 axis, using the same mechanism §15 already built (no new fields, no new modifiers) — `matchExerciseName`/`matchRowLabel`, `applyGripRotationModifier`, `applyGripWidthModifier`, and the `limb` single/bilateral flag all reused as-is.

**Folded (18 entries, all `supersededBy: 'row'`):** Barbell Row (Overhand/Underhand), Meadows Row, Dumbbell Row (Three-Point/Chest-Supported/Pronated), Bent-Over Dumbbell Row (Bilateral), Seated Cable Row, Single-Arm Cable Row, Standing Single-Arm Cable Row, High Cable Row, Wide-Grip Cable Row, T-Bar Row, Machine Row (Seated), Single-Arm Landmine Row, Rack Row (Barbell Inverted), Inverted Row (TRX/Rings), Chest-Supported Barbell Row. Exact angle/equipment/rotation/limb values, and the reasoning behind each, are documented in `functions/pressRowMigration.js`'s `LEGACY_ROW_NAME_MAP` (not re-derived here) — same non-re-derivation convention §15 itself used for its own table.

**Confirmed excluded, per this section's own scrutiny of the task's candidate list:** **Upright Row** — own curveNote ("similar to lateral raise profile"), `category: 'shoulders'`, primary `mid-delt` — belongs to §18's Transverse/Sweep axis, not Row's. Also (not in the task's candidate list, but re-confirmed here for completeness): **Face Pull** and **Kelso Shrug** — both already excluded by §15's own scope note, for the same "mechanically distinct, not an angle point" reasoning (Face Pull's rotator-cuff-external-rotation component and Kelso Shrug's fixed scapular-elevation axis have no honest single point on the Row pull-direction axis).

**Decisions this pass made that the task's own brief hadn't fully resolved** (flagged explicitly, per §19's own convention):

- **Barbell Row Overhand/Underhand share one angle (60°), differentiated by grip rotation, not two different angles.** The pre-existing bulk `exerciseEmgProfiles.js` data placed them at two different angles (60/45) to approximate the grip difference, but that gave Underhand/Yates *less* biceps credit than Overhand — backwards from its own curveNote ("Supinated grip maximises bicep involvement"). Now that rotation (§11) is a real modifier, the correct fix reuses it instead of preserving an angle-table workaround that predates it.
- **High Cable Row / Wide-Grip Cable Row corrected to 105°, not the bulk-profile-implied 120°.** 120° is `ROW_PULLDOWN_ANGLE_THRESHOLD` — migrating either at that exact value would make `matchRowLabel` display them as "Lat Pulldown," which neither curveNote supports (both describe pulling elbows back to the torso, not pulling a bar down to a fixed low point).
- **T-Bar Row's angle (75°) and equipment (Machine) both override the bulk `exerciseEmgProfiles.js` default (60°, Barbell)**, in favor of `functions/exerciseAngles.js`'s own dedicated hand-curated entry (angle) and §13's own T-Bar Row resolution (equipment) — both individually-curated, higher-confidence sources than the generic bulk profile pass.
- **§13's T-Bar Row "Plate-Loaded Machine" classification extended to Meadows Row and Single-Arm Landmine Row** (both landmine-anchored, same fixed-pivot mechanical category) — not re-litigated per entry, since §13's own reasoning ("the lateral freedom... doesn't meaningfully change its stability category") applies equally to any landmine setup, not just the T-bar handle specifically.
- **Rack Row and Inverted Row classified as Bodyweight equipment**, not Barbell — the bar/rings are the anchor, not the load source, same classification the pull-up/chin-up cluster already uses despite a bar being physically involved.
- **Meadows Row and Single-Arm Landmine Row both get `limb: 'single'`** despite neither being named "Single-Arm" — both entries' own form cues ("grip end of bar" / "Brace contralateral knee and hand") describe a one-arm pull. Limb classification follows the exercise's own description, not its name.
- **Dumbbell Row (Pronated) corrected to angle 60 (matching the plain dumbbell-row cluster) plus rotation, not the bulk-profile-implied 75°** — its own curveNote is entirely about grip, not pull direction; the 75° bucket it shared with Rack Row/Inverted Row in the bulk data looks coincidental, not deliberate.
- **Chest-Supported Dumbbell Row flagged `needsClarification`, not auto-migrated** — its own form cue explicitly allows "Both arms simultaneously or alternating," a genuine bilateral/single-limb fork, same policy §4 already established for Dumbbell Overhead Press's stance ambiguity.
- **Cable Straight-Arm Pulldown, Face Pull, Upright Row, and Kelso Shrug remain untouched**, per §15's own carve-out and this section's re-confirmation above.
- **Seated/standing-machine rows (Seated Cable Row, Single-Arm Cable Row, Machine Row, High/Wide Cable Row) were left with no `stance` value** in the migration table — Row's current `stanceOptions` (`Standing`/`Chest-Supported`) doesn't cleanly describe a seated-at-a-machine position either way, and forcing one would misrepresent it. Flagged as an open gap for whenever Stance/Support's fatigue-crediting mechanism (§12, still not built) gets picked up — it may need a third value, not just a fix to these entries' data.
- **`functions/strengthStandards.js`'s `classifyLift` gained a `'row'` entry in `AMBIGUOUS_EQUIPMENT_ALLOWLIST`** (Barbell equipment + exact angle 60), the same fix §14 made for Bench Press/Overhead Press — without it, a *new* log of the classic bent-over barbell row (now saved as canonical `Row` + Barbell + 60°, since the two literal names that used to trigger this are retired from the picker) would silently stop counting toward the `/trends` chart and PR banner's "5 classic lifts" `row` category.

**Not attempted this pass:** no further Row entries beyond the task's 18-name candidate list were audited — Kelso Shrug/Face Pull's exclusion above is a re-confirmation of §15's existing reasoning, not a fresh audit of entries outside the given scope.

## 23. Legs — scoping pass (§2 item 5), not yet a build spec

§2 explicitly deferred this: "quad/hamstring/glute emphasis is real but mostly plays out as genuinely different exercises... rather than one parameterized movement with a slider... needs its own scoping pass later, not assumed to fit this model." This section is that pass — it maps the terrain and flags real open questions, it does not lock a spec the way §14/§15/§20/§21 did, because the underlying literature situation here is genuinely different from Press/Row/Curl/Extension's "zero new sourcing" position (§17 marked Legs "new sourcing, but scoped and named" at best, "not yet scoped at all" at worst).

**A structural difference from every pattern parameterized so far, worth stating plainly**: `functions/movementEmg.js`'s existing `SQUAT_HIP_EMG`, `HINGE_HIP_EMG`, and `KNEE_EMG` tables are NOT the same kind of table as `PRESS_EMG`/`ROW_EMG`/`ELBOW_SHOULDER_MODIFIER`. Those model a REP-PHASE axis (how activation changes as you descend/ascend through ONE rep of a fixed exercise — 0°=lockout through 90°=bottom of a squat), not an EXERCISE-VARIANT axis (a fixed setup choice made once per exercise instance, like Press's incline or Curl's shoulder position). This matters because it means "depth" as a per-exercise picker field (the naive reading of §17's Depth/ROM column) doesn't map cleanly onto data that already varies continuously within a single set anyway — a squat's "depth" is closer to a per-rep quality (how low you actually go) than a per-exercise-instance choice, unlike Press's incline which is fixed for the whole set. Whether "depth" should even be a stored picker field, vs. left as the existing named-variant distinction (Box Squat/Pin Squat/Pause Squat already exist as separate DB entries specifically because they truncate depth/ROM in different ways) is a real open product question, not a data question.

**Per-pattern read of what's tractable vs. not, cross-referencing §17's own table:**

- **Squat stance width** — plausibly the most tractable axis to actually build. Real named variants already exist on a clear physical spectrum (narrow/shoulder-width stance through `sumo-squat`), several curveNotes already describe the width→adductor/quad tradeoff directionally (e.g. Sumo Squat's "wide stance targets adductors... inner thigh"), and stance-width EMG (adductor magnus, quads, glutes) is a real, published, comparatively well-studied area (similar evidentiary shape to the grip-width row literature already cited in §10's `ROW_GRIP_WIDTH_EMG`) — worth a literature pass before committing to specific numbers, not worth guessing at directly here.
- **Deadlift stance width AND knee-bend/torso-angle are BOTH real, and not the same axis** — Conventional/Sumo differ primarily by stance width (same tractable literature situation as squat); Romanian/Stiff-Leg/Conventional differ primarily by knee bend and torso angle, which is a genuinely different mechanical question (hip-hinge depth/ROM, closer to the rep-phase `HINGE_HIP_EMG` table's own domain than a fixed picker field). Collapsing Deadlift onto ONE parameterized axis the way Press collapsed onto incline would be a mistake — this family needs at least two distinguishable questions answered, possibly two separate fields, and is not a copy-paste of the Press/Row playbook.
- **Leg Press** — §17 marks Sagittal Angle, Width, AND Depth all `Y`, all genuinely unsourced: no equivalent to `SQUAT_HIP_EMG` exists for Leg Press specifically (foot-platform angle changes the muscle emphasis differently than free-squat hip angle does — higher/narrower foot placement shifts toward quads, lower/wider toward glutes/hamstrings/adductors, but this app has no curated table for it at all yet). Real, scoped, but new sourcing work, not a quick wire-up.
- **Leg Curl (Hamstring)** — already has real sourced data (Maeo et al. 2020, hip-position-dependent hamstring activation, cited in §17's own notes) and a small, closed set of named variants (Lying/Seated/Standing/Nordic/Swiss-Ball) that map fairly cleanly onto "hip position" as the one real axis. Of everything in this section, this is the closest to Curl/Extension's "zero new sourcing" position — a real near-term candidate once Squat/Deadlift's bigger open questions are resolved, or even before them given how self-contained it is.
- **Leg Extension** — §17 marks every column `N` except Equipment. Genuinely nothing to parameterize here beyond what already exists; not a gap, a correctly-scoped "no".
- **Hip Thrust / Kickback** — §17 marks Depth (Hip Thrust) and Frontal-Plane Angle (Kickback) as the only real axes, both flagged "not yet scoped at all." Lowest priority of this whole section — small named-variant counts, thin existing signal to audit against, no literature identified yet even informally.
- **Calf Raise** — §17 marks Depth only. Named variants (Standing/Seated/Single-Leg/Donkey/Leg-Press) already differ mostly by equipment and stance, not a depth spectrum with real gradation described in their curveNotes — likely stays as-is; flagged rather than assumed.

**Open questions for George before any build work starts here** (unlike Press/Row/Curl/Extension, this family's literature situation doesn't support the same "propose specific numbers, correct me" pass this doc used before — the gaps are bigger and the axes themselves, not just their boundary values, need a decision):
1. Is Squat stance-width (narrow/shoulder-width/sumo) worth a literature-research pass now, as the most tractable Legs axis — similar to how the Row grip-width literature review currently in flight might inform this same kind of pass?
2. For Deadlift: is it worth treating stance-width and knee-bend/hinge-depth as two SEPARATE fields on one collapsed `deadlift` entity, or does that overcomplicate a family where Conventional/Sumo/RDL/Stiff-Leg arguably still deserve to stay separately-named (more like Arnold Press/Push Press's carve-out status than like Bench Press's collapse)?
3. Is Leg Curl's hip-position axis (already sourced, small variant set) worth building as a standalone near-term piece, independent of Squat/Deadlift's bigger open questions?
4. Leg Press's own angle+width+depth table is real, scoped, new-sourcing work comparable in shape to the original `PRESS_EMG`/`ROW_EMG` curation effort (per §5's own comparison) — worth scheduling as its own dedicated pass, or lower priority than finishing Row's remaining collapse (§15's deferred ~15-entry scope) and Phase 4's coaching layer (§3)?
