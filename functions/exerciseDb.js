...
//
// Taxonomy fields (muscleGroup/pattern/movementId/movementName) added
// programmatically in one pass, not hand-typed per entry — see git history
// for the generation approach. Purely additive metadata for browsing/
// grouping; every existing field (name, primary/secondary, equipment) is
// untouched, so nothing about exercise identity, muscle attribution, or
// historical logged-data matching changed.
//
// - muscleGroup: derived from primary[0] (this DB already orders `primary`
//   by dominance, e.g. Close-Grip Bench Press lists triceps before chest
//   because that variant really is triceps-dominant), with a small number of
//   hand-overridden exceptions where the strict rule mismatches lifter
//   intuition for a compound lift (Sumo Squat, Adductor Squeeze Squat,
//   Jefferson Curl — see EXERCISE_MUSCLE_GROUPS list).
// - pattern: the movement's true biomechanical shape, not a keyword match on
//   the name — e.g. Leg Press is "squat" (not "press", despite the name);
//   Pallof Press is "rotation" (anti-rotation core work, not a press).
// - movementId/movementName: the "parent exercise" shared by every
//   equipment/angle variant (e.g. Barbell/Dumbbell/Incline/Decline Bench
//   Press all share movementId "bench-press") — groups variants together
...
// files import both modules; reusing the name would silently shadow one.
const EXERCISE_MUSCLE_GROUPS = [
  'chest', 'back', 'shoulders', 'biceps', 'triceps', 'forearms',
  'quads', 'hamstrings', 'glutes', 'calves', 'hips', 'core',
];

// Every pattern value any exercise uses — finite and closed, same as the
// muscle-group list, but not tied 1:1 to any single muscle group (e.g.
// "press" spans Chest and Shoulders; "curl" spans Biceps and Hamstrings/leg
// curls). Extend by adding a new word here + tagging the relevant
// exercise(s), same mechanism as adding a new equipment brand to
// machineBrands.js.
const EXERCISE_PATTERNS = [
  'press', 'fly', 'pulldown', 'pullover', 'row', 'shrug', 'extension', 'curl',
  'raise', 'squat', 'lunge', 'hinge', 'thrust', 'calf-raise', 'adduction',
  'abduction', 'rotation', 'crunch', 'hold', 'rollout', 'carry', 'clean', 'dip',
  // 'leg-curl'/'hyperextension', not 'curl'/'extension' -- distinct from
  // those on purpose, see functions/emgActivation.js's header note on the
  // pattern-name overload between arm and leg movements.
  'leg-curl', 'hyperextension',
];
...
const EXERCISE_DB = [
  ...
  },
  {
    ...
    lesserKnown: true,
    muscleGroup: "chest", pattern: "press", movementId: "svend-press", movementName: "Svend Press"
  },
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "glutes", pattern: "hinge", movementId: "pull-through", movementName: "Cable Pull-Through"
  },
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "triceps", pattern: "press", movementId: "diamond-push-up", movementName: "Diamond Push-Up"
  },
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "quads", pattern: "lunge", movementId: "step-up", movementName: "Step-Up"
  },
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "hips", pattern: "abduction", movementId: "clamshell", movementName: "Clamshell"
  },
  ...
  },
  {
    ...
    lesserKnown: false, isometric: true,
    muscleGroup: "core", pattern: "hold", movementId: "hollow-body-hold", movementName: "Hollow Body Hold"
  },
  // ── ADDITIONAL CHEST ─────────────────────────────────────────────────────────
  {
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "chest", pattern: "press", movementId: "single-arm-cable-press", movementName: "Single-Arm Cable Press"
  },
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "quads", pattern: "squat", movementId: "jump-squat", movementName: "Jump Squat"
  },
  ...
  },
  {
    ...
    lesserKnown: false,
    muscleGroup: "core", pattern: "rotation", movementId: "woodchop", movementName: "Woodchop"
  },
  ...
  // build it at (functions/emgActivation.js's PRESS_EMG/ROW_EMG/FLY_EMG,
  // 0-180° in 15° steps) — angle is a per-set attribute (see PressRowBuilder
  // in src/app.jsx), not part of exercise identity, so all your logged sets
  // across every angle share one history here. primary/secondary are
  // hand-curated for realistic session-picker weighting (chest first for a
  // press, not whichever muscle's own peak % happens to be numerically
  // largest — this file's own EMG tables use each muscle's own MVIC
  // reference, not a shared scale) but every muscle listed is confirmed to
  // actually cross PRIMARY_THRESHOLD/SECONDARY_THRESHOLD somewhere in the
  // pattern's table via functions/emgActivation.js's muscleEnvelope(pattern)
  // — see test/exerciseDb.test.js. The precise per-set muscle credit always
...
module.exports = { EXERCISE_DB, EXERCISE_MAP, EXERCISE_MUSCLE_GROUPS, EXERCISE_PATTERNS };
