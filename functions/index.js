...
function computeStructuralFatigue(lifts, peaks, soreness = [], sensitivity = {}, recoveryHours = RECOVERY_H_B) {
  const now = Date.now();
  const scores = {};
  for (const l of (lifts || [])) {
    ...
    for (const [key, muscles] of Object.entries(MUSCLE_MAP_B)) {
      if (name.includes(key)) {
        muscles.forEach(m => {
          const hl = recoveryHours[m] || RECOVERY_H_B[m] || 72;
...
function computeCurrentFatigueScores(lifts, peaks, soreness = [], sensitivity = {}, recoveryHours = RECOVERY_H_B) {
